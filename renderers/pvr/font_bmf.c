...
typedef struct __attribute__((__packed__)) bm_info {
  ...
  uint8_t spacingVert;
  uint8_t outline;
  // char fontName[/*n+1*/]; /* VLA, size of block length - 14, null terminated
...
static int BMF_parse_info(FD_TYPE fd, size_t block_size, bm_font *font) {
  ...
  bm_info temp_info;
  // fread(&temp_info, block_size, 1, fd);
  fread(&temp_info, INFO_NAME_OFFSET, 1, fd);
  fread(_fontName, block_size - INFO_NAME_OFFSET, 1, fd);

  /* Fix fontSize, its negative ? */
  temp_info.fontSize *= -1;

  /* fix fontName */
  temp_info.fontName = _fontName;
...
static int BMF_load(const char *file, bm_font *font) {
  ...
  */
  ...
  while (parsing) {
    ...
    DBG_PRINT("Found block type %d of size %ld read %ld\n", next_block.type,
              next_block.size, ele_read);
#ifdef GDROM_FS
...
int BMF_adjust_kerning(unsigned char first, unsigned char second,
                       bm_font *font) {
...
static int z_get(void) { return z; }
void z_set(int newZ) { z = newZ; }
static void z_inc(void) {}
static float current_scale = 0.f;
static unsigned int current_color = 0;

void font_bmf_set_height_default(void) { current_scale = 1.f; }
void font_bmf_set_height(float height) {
  current_scale = height / (float)font_basilea.fontSize;
}
void font_bmf_set_scale(float scale) { current_scale = scale; }

/* Scaling */
#define X_SCALE_4_3 (1.0f)
#define X_SCALE_16_9 (0.74941452f)
static float X_SCALE = X_SCALE_4_3;
...
#ifdef KOS_SPRITE
static pvr_sprite_txr_t charbuf[BUFFER_MAX_CHARS * VERT_PER_CHAR]
    __attribute__((aligned(32)));
#else
static pvr_vertex_t charbuf[BUFFER_MAX_CHARS * VERT_PER_CHAR]
    __attribute__((aligned(32)));
#endif
...
static int font_bmf_draw_char(int x, int y, unsigned char chr) {
  ...
  /* Upper left */
  const float x1 =
      round(x + (current_scale * (float)font->chars[chr].xoffset) * X_SCALE);
  ...
  /* Lower right */
  const float x2 = round(x + (current_scale * ((float)font->chars[chr].width +
                                               font->chars[chr].xoffset)) *
                                 X_SCALE);
  const float y2 = round(y + current_scale * ((float)font->chars[chr].height +
                                              font->chars[chr].yoffset));
  const float u2 =
      (float)(font->chars[chr].x + font->chars[chr].width) / (float)font->width;
  const float v2 = (float)(font->chars[chr].y + font->chars[chr].height) /
                   (float)font->height;
...
static void _font_bmf_draw_string(int x1, int y1, uint32_t color,
                                  const char *str) {
  current_color = color;
  charbuffered = 0;
  z_inc();
...
static void _font_bmf_draw_string_slice(int x1, int y1, uint32_t color,
...
static float _font_bmf_calculate_length_full(const char *str, int length) {
  ...
  return roundf(current_scale * roundf(width)) * X_SCALE;
}
...
void font_bmf_draw_auto_size(int x1, int y1, uint32_t color, const char *str,
                             int width) {
...
void font_bmf_draw_centered_auto_size(int x1, int y1, uint32_t color,
...
void font_bmf_draw_sub_wrap(int x1, int y1, uint32_t color, const char *str,
                            int width) {
  z_inc();
  current_color = color;
  unsigned char prev = ' ';
  ...
  do {
    ...
    do {
      ...
      current_text_width +=
          (int)((current_scale * font_basilea.chars[current_char].xadvance) *
                X_SCALE);
      current_text_width +=
          round(current_scale *
                BMF_adjust_kerning(prev, current_char, &font_basilea));
      prev = current_char;
      current_text_len++;
    ...
    do {
      ...
      if (chr != ' ') {
        /* Add possible kerning adjustment */
        x1 +=
            round(current_scale * BMF_adjust_kerning(prev, chr, &font_basilea));
        x1 += font_bmf_draw_char(x1, y1, chr);
      } else {
        x1 += round(current_scale * (float)font_basilea.chars[' '].width);
      }
...
float font_bmf_calculate_length_slice(const char *str, int length) {
  return _font_bmf_calculate_length_full(str, length);
}

void font_bmf_draw_slice(int x, int y, uint32_t color, const char *str,
                         int length) {
  _font_bmf_draw_string_slice(x, y, color, str, length);
}

float font_bmf_get_original_height(void) {
  return (float)font_basilea.fontSize;
}
float font_bmf_get_current_height(void) {
  return current_scale * (float)font_basilea.fontSize;
}
