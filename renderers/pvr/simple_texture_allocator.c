...
static intptr_t tex_buffer = (intptr_t)NULL;
static intptr_t tex_buffer_start = (intptr_t)NULL;
static intptr_t tex_buffer_max = (intptr_t)NULL;
...
void texman_reset(intptr_t buf, uint32_t size) {
...
void texman_set_buffer(intptr_t buf, uint32_t size) {
  tex_buffer = buf;
  tex_buffer_max = buf + size;
}
...
intptr_t texman_get_tex_data(uint32_t num) { return textures[num].location; }
...
struct Simple_Texture *texman_reserve_memory(uint32_t width, uint32_t height,
  ...
  if (texman_is_space_available()) {
    ...
    tex_buffer =
        (intptr_t)((((size_t)tex_buffer + tex_size + TEX_ALIGNMENT - 1) /
                    TEX_ALIGNMENT) *
                   TEX_ALIGNMENT);
#ifdef DEBUG
...
void texman_upload(uint32_t width, uint32_t height, int bpp,
  ...
  memcpy((void *)current->location, buffer, getMemorySize(width, height, bpp));
#ifdef DEBUG
